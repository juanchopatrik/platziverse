...
test.serial.cb('/api/agents/:uuid', t => {
  request(server).get(`/api/agents/${uuidArgs}`).expect(200).expect('Content-Type', /json/).end((err, res) => {
    t.falsy(err, 'should not return an error');
    let body = JSON.stringify(res.body);
    let expected = JSON.stringify(agentFixtures.byUuid(uuidArgs));
    t.deepEqual(body, expected, 'response body should be the expected');
    t.end();
  });
});
...
//# sourceMappingURL=/home/juancho/Documentos/platzi/platziverse/platziverse-api/node_modules/.cache/ava/21d88df343c049b8fafa823e60d584f0.js.map